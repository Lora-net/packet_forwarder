...
#define TX_BUFF_SIZE	((540 * NB_PKT_MAX) + 30 + STATUS_SIZE) /* TODO: not used here, but used in gps_pkt_fwd, why? */
...
static int parse_gateway_configuration(const char * conf_file) {
	...
	/* network identifier (aka NetID) (optional) */
	str = json_object_get_string(conf_obj, "network_ID");
	if (str != NULL) {
		sscanf(str, "%llx", &ull);
        netID = ull;
		MSG("INFO: network ID is configured to %06llX\n", ull);
	}
	...
	while (!exit_sig && !quit_sig) {
		...
		for (i=0; i < nb_pkt; ++i) {
            ...
            if (ref_ok == true) {
                ...
                MSG("thread_up: ref.count_us=%u : p->count_us=%u\n", local_ref.count_us, p->count_us);
                if (j == LGW_GPS_SUCCESS) {
                    pkt_gps_time_ms = pkt_gps_time.tv_sec * 1E3 + pkt_gps_time.tv_nsec / 1E6;
                    j = snprintf((char *)(buff_up + buff_index), TX_BUFF_SIZE-buff_index, ",\"time\":%llu",
                                    pkt_gps_time_ms); /* GPS time in milliseconds since 01.01.1970 */
                    if (j > 0) {
                        buff_index += j;
                    } else {
                        MSG("ERROR: [up] snprintf failed line %u\n", (__LINE__ - 4));
...
void thread_down(void) {
	...
	*(uint32_t *)(buff_req + 8) = net_mac_l;
    ...
    nwkID = netID & 0x7F; /* get 7 LSB of NetID */
	...
	beacon_pkt.payload[0] = 0xFF &  netID;
	beacon_pkt.payload[1] = 0xFF & (netID >>  8);
	beacon_pkt.payload[2] = 0xFF & (netID >> 16);
	/* 3-6 : time (variable) */
	/* 7 : crc1 (variable) */
		...
		while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
            ...
            while (beacon_loop && (beacon_period != 0)) {
                ...
                if ((gps_ref_valid == true) && (xtal_correct_ok == true)) {
                    ...
                    /*   LoRaWAN: T = k*beacon_period + NwkID + TBeaconDelay */
                    /*            with NwkID = [0:127s]                      */
                    /*                 TBeaconDelay = [0:50ms]               */
                    if (last_beacon_gps_time.tv_sec == 0) {
                        /* if no beacon has been queued, get next slot from current GPS time */
                        diff_beacon_time = time_reference_gps.gps_time.tv_sec % ((time_t)beacon_period);
                        next_beacon_gps_time.tv_sec = time_reference_gps.gps_time.tv_sec +
                                                        ((time_t)beacon_period - diff_beacon_time);
                        /* add the nwkID offset to the previously calculated time for next beacon */
                        /* from there, we have our beacon time reference */
                        next_beacon_gps_time.tv_sec += nwkID; /* add "nwkID" seconds */
                    } else {
                        /* if there is already a beacon, take it as reference */
                        next_beacon_gps_time.tv_sec = last_beacon_gps_time.tv_sec;
                    }
                    ...
                    MSG_DEBUG(DEBUG_BEACON, "GPS-now : %s", ctime(&time_reference_gps.gps_time.tv_sec));
					...
					beacon_pkt.payload[3] = 0xFF &  next_beacon_gps_time.tv_sec;
					beacon_pkt.payload[4] = 0xFF & (next_beacon_gps_time.tv_sec >>  8);
					beacon_pkt.payload[5] = 0xFF & (next_beacon_gps_time.tv_sec >> 16);
					beacon_pkt.payload[6] = 0xFF & (next_beacon_gps_time.tv_sec >> 24);
			...
			/* Parse "immediate" tag, or target timestamp, or GPS time to be converted by GPS (mandatory) */
			...
			if (i == 1) {
				...
				if (val != NULL) {
					...
					val = json_object_get_value(txpk_obj, "time");
					if (val == NULL) {
...
void thread_gps(void) {
    ...
    char *serial_buff_ptr; /* start position in the buffer */
	ssize_t nb_char;
	ssize_t nb_char_msg;
	...
	while (!exit_sig && !quit_sig) {
        ...
        /* Initialize parsing start position */
        serial_buff_ptr = serial_buff;

        /* First check if there are some UBX messages (binary) in buffer */
        /* Note: Assuming that UBX messages will be before NMEA message */
        /* We have only one NMEA message per line, and could have several */
        /*  UBX binary messages */
        /* TODO: make parsing smarter: here we expect sync chars to be at the beginning */
        while (nb_char > 0) {
            latest_msg = lgw_parse_ubx(serial_buff_ptr, nb_char, &nb_char_msg);
            if (nb_char_msg > 0) {
                /* Update parsing values */
                serial_buff_ptr += nb_char_msg;
                nb_char -= nb_char_msg;
                /* trigger sync only on UBX_NAV_TIMEGPS frames */
                if (latest_msg == UBX_NAV_TIMEGPS) {
                    /* get GPS time for synchronization */
                    i = lgw_gps_get(&gps_time, NULL, NULL);
                    if (i != LGW_GPS_SUCCESS) {
                    ...
                    pthread_mutex_lock(&mx_timeref);
                    i = lgw_gps_sync(&time_reference_gps, trig_tstamp, gps_time);
                    pthread_mutex_unlock(&mx_timeref);
                    if (i != LGW_GPS_SUCCESS) {
                ...
                break; /* no more UBX message, we can exit from this loop */
            }
        }

        /* Then check if there is some NMEA message in the buffer */
        if (nb_char > 0) {
            /* parse the received NMEA */
            latest_msg = lgw_parse_nmea(serial_buff_ptr, nb_char);

            if (latest_msg == NMEA_RMC) { /* Get location from RMC frames */
